...
export async function initRedis() {
  ...
  try {
    ...
    redis = new Redis(REDIS_URL, {
      ...
      retryStrategy(times) {
        if (times > 3) {
          console.error('❌ Redis connection failed after 3 retries - falling back to MongoDB');
...
export async function getWorkspaceByInstagramId(
  instagramId: string,
  db: Db
): Promise<any> {
  ...
  // Try account lookup
  let account = await db.collection('instagram_accounts').findOne({
    $or: [
