...
export async function GET(request: NextRequest, { params }: { params: { workspaceId: string } }) {
  try {
    ...
    const account = await db.collection("instagram_accounts").findOne({ instagramId: workspace.instagramId })
    if (!account) {
      return NextResponse.json({ error: "Instagram account not found" }, { status: 404 })
    }

    // Fetch user profile info
    const userRes = await fetch(`https://graph.instagram.com/me?fields=id,name,username,account_type,media_count&access_token=${account.accessToken}`)
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        instagramId: workspace.instagramId,
