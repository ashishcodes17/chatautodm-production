...
export async function POST(request: NextRequest, { params }: { params: { workspaceId: string } }) {
  try {
    console.log("[v0] Comment flow API called with workspaceId:", params.workspaceId)
    ...
    console.log("[v0] Received automation data:", JSON.stringify(automation, null, 2))
    ...
    if (!workspace) {
      console.log("[v0] Workspace not found for:", workspaceId, "user:", user.id)
      return NextResponse.json({ success: false, error: "Workspace not found" }, { status: 404 })
    }

    console.log("[v0] Found workspace:", workspace._id)
    ...
    if (!account) {
      console.log("[v0] Instagram account not found for workspace:", workspaceId)
    ...
    console.log("[v0] Saving comment automation:", JSON.stringify(commentAutomation, null, 2))

    // ✅ Step 5: Save automation
    const result = await db.collection("automations").insertOne(commentAutomation)

    console.log("[v0] Comment automation saved with ID:", result.insertedId)

    return NextResponse.json({
      success: true,
      automationId: result.insertedId,
      message: "Comment automation created successfully",
    })
  } catch (error) {
