...
export default function Page() {
  ...
  const [automation, setAutomation] = useState<DMAutomationState>({
    ...
    storyId: null,
    isActive: true, // flow status toggleable
  })

  const [keywordInput, setKeywordInput] = useState("")

  // Add keyword helper to reuse for button click and Enter key
  const addKeyword = () => {
    const kw = keywordInput.trim()
    if (!kw) return
    if (!automation.trigger.keywords.includes(kw)) {
      setAutomation({
        ...automation,
        trigger: {
          ...automation.trigger,
          keywords: [...automation.trigger.keywords, kw],
        },
      })
    }
    setKeywordInput("")
  ...
  const normalizeAutomationFromApi = (apiAutomation: any): DMAutomationState => {
    const mapButtonsBack = (buttons: any[] = []): DMButton[] =>
      buttons.map((b: any): DMButton => {
        if (b?.type === "web_url") {
    ...
    return {
      ...
      isActive: apiAutomation?.isActive !== false,
    }
  }
  ...
  const renderMobilePreview = () => {
    if (activeStep === 0) {
      ...
      // Chat view for steps 1, 2, 3, and 4
      return (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {automation.isActive
                  ? "Live - the automation will respond to DMs."
                  : "Paused - the automation will not respond."}
              </p>
            </div>
            <Switch
        ...
        >
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <input
              ...
              className="w-full border border-gray-300 rounded-md px-3 pr-20 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
            <button
              type="button"
              ...
              disabled={!keywordInput.trim() || automation.trigger.keywords.includes(keywordInput.trim())}
              className="absolute right-1.5 top-1/2 -translate-y-1/2 inline-flex items-center justify-center h-8 w-8 rounded-md bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
