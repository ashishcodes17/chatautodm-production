...
async function testRedisIntegration() {
  ...
  try {
    ...
    const workspaces = await db.collection('workspaces').find({ isActive: true }).limit(5).toArray();
    console.log( Found  workspaces\n);

    if (workspaces.length === 0) {
      console.log('  No workspaces found, skipping cache test');
      return;
    }
    ...
    for (const automation of automations) {
      const autoKey = utomation:::all;
      await redis.setex(autoKey, 3600, JSON.stringify([automation]));
    }
    console.log(   Cached  automations\n);
    ...
    console.log(   - Workspaces cached: );
    console.log(   - Automations cached: );
    console.log(   - Cache retrieval: ms (vs ~20ms MongoDB));
    console.log('');
